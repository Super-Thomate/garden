...
class Rules(commands.Cog):
  ...
  async def is_emoji_valid(self, msg_emoji: discord.Message):
    try:
      ...
      await msg_emoji.channel.send (Utils.get_text (msg_emoji.guild.id, "rule_no_emoji").format (msg_emoji.content))
      return False
  ...
  async def get_rule(self, ctx: commands.Context, emoji_text: str = None):
    ...
    if not fetched:
      response = await ctx.send(Utils.get_text(ctx.guild.id, "no_rules_found").format(emoji_text))
