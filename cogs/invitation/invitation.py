...
class Invitation(commands.Cog):
  ...
  async def set_invite_channel(self, ctx, channel: discord.TextChannel = None):
    ...
    database.execute_order(sql)
    await invite_channel.send(Utils.get_text(ctx.guild.id, "invitation_channel_set"))
  ...
  async def get_invitation_link(self, guild_id):
    url = Utils.invite_url(guild_id) or botconfig.config[str(guild_id)]['create_url'][
      'invitation']  # build the web adress
